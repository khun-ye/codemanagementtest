...
@Composable
fun UpCommingMoviesScreen(
    viewModel: UpCommingViewModel = hiltViewModel(),
    navController: NavController
) {

    val upcommingMovies = viewModel.upcommingMovies.collectAsLazyPagingItems()
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            items(upcommingMovies) { popular ->
                popular?.let {
                    UpCommingMoviesItem(popular = popular) { navigated ->
                        navController.navigate(Screen.MovieDetail.route + "/${navigated.id}")
                    }
                }
            }
            upcommingMovies.apply {
                when {
...
@Composable
fun UpCommingMoviesItem(popular: Movie, onClick: (Movie) -> Unit) {
